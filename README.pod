...
- for each unique word, find all the words that follow it in the corpus and stick that in a hash along with a <abend> token
...
- index into hash, rand() index into array to find next one (ending if it's <abend>)

- if it wasn't <abend>, repeat with the new token
