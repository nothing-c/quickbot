...
- weighted/unweighted mode; unweighted is the current one, where each unique word only appears once; weighted means that each unique word appears the # of times it appears in the corpus - make this a commandline option
